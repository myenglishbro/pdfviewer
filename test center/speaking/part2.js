(() => {
  const DATA_URL = '../../assets/data/speaking-part2.json';
  ...
  function normalizePdf(url) {
    ...
    if (drive) return `https://drive.google.com/file/d/${drive[1]}/preview#toolbar=0&navpanes=0`;
  ...
  function renderPrompt(nextIdx) {
    ...
    const pdfSrc = normalizePdf(item.pdf || item.image);
    if (pdfFrame) pdfFrame.src = pdfSrc || '';
    imgTitle.textContent = item.title || `Prompt ${idx + 1}`;
    imgTag.textContent = item.tag || `Task ${idx + 1}`;
    promptTitle.textContent = item.title || `Prompt ${idx + 1}`;
    promptBody.textContent = item.instruction || 'Describe la imagen.';
  ...
  function stopTimer(resetToPrompt = false) {
    ...
    if (resetToPrompt) {
      const secs = Number(prompts[idx]?.timeLimitSec) || 60;
      timerChip.textContent = formatTime(secs);
      timerBar.style.width = '0%';
    }
  }
  ...
  pdfOverlay?.addEventListener('click', () => {
    ...
    if (!src || !zoomModal || !zoomFrame) return;
    zoomFrame.src = src;
    zoomModal.classList.add('is-open');
  });

  zoomClose?.addEventListener('click', () => {
    if (!zoomModal || !zoomFrame) return;
    zoomFrame.src = '';
